...
{	
		...
		.done( function( data )
		{
			if( data.result )
			{
				...
				{	
					...
					col3 = value.email;
					col4 = value.depto;
					col5 = '<a class="btn btn-danger btn-xs btn-eliminar" href="#modal-eliminar" data-toggle="modal"  data-idregistro="'+value.id+'"><i class="fa fa-trash-o "></i></a>';
	...
	$("#tabla-registros").on('click','a.btn-editar', function( e )
	{
	    ...
	    $.ajax({
	        type: "GET",
	        url : "ws/usuarioscomedores/"+idregistro,
	        success : function( result )
        	{
				if( result.result )
				{
	...
	function update( e )
	{
		...
		if( $('#form-editar').valid() )
		{
			...
			({
				type: 		'PUT',
				url: 		'ws/usuariosconcursos/'+idregistro,
				dataType: 	'json',
	...
	function destroy( e )
	{
		...
		({
			type:	"DELETE",
			url:	"ws/usuariosconcursos/"+ideliminar,
			success: function( result )
			{
				if( result.result )
				{
