jQuery(document).ready(function($){
    ...
    window.tabla_registros = $("#tabla-registros").DataTable({
        ...
        processing: true,
        //serverSide: true,
        ajax: 'ws/activos/equipos/datatable',
        columns: [
         ...
         'columnDefs': [
            { 'searchable': false, 'targets': [0,7,8,18] }
          ]
    });
...
function limpiar_validacion(){
    ...
    function reasignar(){
        var uid_empleado = $("#form-reasignar #uid_empleado_reasignar").val().trim();
        
        if (uid_empleado.length == 0) {
            return toastr['warning']('El campo UID no puede quedar vacío');
        }
        ...
        .done(function(response){
            if (response.result) {
                ...
                var data = tabla_registros.row(index).data();
                ...
                tabla_registros.row(index).data(data).draw();
            } else {
                toastr['error'](response.message, 'Error');
            }
        })
