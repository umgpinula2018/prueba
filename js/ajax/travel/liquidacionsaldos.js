...
function iniciar()
{
	...
	function BD_Cargar(){
		...
		jQuery.ajax({
			url:'ws/travel/liquidacion/todas',	
			type:'POST', 	
			dataType: 'json',
			...
			if(data.result){
				...
				jQuery.each(consulta, function (index, valor) { 
					// if( valor.estado == 1 ){
						...
						var estado = '';
						var jefe = '';
						switch(parseInt(valor.moneda)){
	                        case 0: mon = 'Dolares'; break;
	                        case 1: mon = 'Colones'; break;
	                        case 2: mon = 'Quetzales'; break;
	                    }
	                    switch(parseInt(valor.estado)){
	                        case 1: estado = 'Aprobada'; jefe=valor.nombre_jefe; break;
	                        case 7: estado = 'Aprobada'; jefe=valor.nombre_jefe; break;
	                        case 9: estado = 'Aprobada'; jefe=valor.nombre_jefe; break;
	                        case 15: estado = 'Completada'; jefe=valor.nombre_jefe; break;
	                        case 2: estado = 'Rechazada'; jefe=' - - - '; valor.fecha=' - - - '; break;
	                        default: estado = 'Pendiente'; jefe=' - - - '; valor.fecha=' - - - '; break;

	                    }
	                    numero_gasto = valor.numero_gasto;
	                    ceco = valor.ceco;
	                    cuenta_gasto = valor.cuenta_gasto;

						row1='<center>'+valor.correlativo+'</center>';
						row2 = estado;
						row3 = valor.nombre_creo;
						row4 = jefe;
						row5 = valor.monto;
						row6 = mon;
						row7 = valor.justificacion;
						row8 = valor.fecha_liquidacion;
						row9 = valor.fecha_finalizacion;
						row10 = valor.updated_at;
						row11 = '<a role="button" title="ver liquidacion" style="margin-left: 5px;" class="btn btn-success btn-xs btn-ver" data-id="'+valor.id+'" href="#modal-ver" data-toggle="modal"><i class="fa fa-download"></i></a>'
		                ...
		                window.tabla_BD.row.add([row1, numero_gasto,row2, ceco, cuenta_gasto,row3,row4,row5,row6,row7,row8,row9,row10,row11]).draw(false);
					// }
					if( index == contar )
	...
	jQuery('#btn-ver-liquidacion').on('click',function(e){
	...
	jQuery('#btn-exportar').on('click',function(e){
		e.preventDefault();
		window.location.href = "ws/travel/liquidaciones/anticipos/exportar";
 ...
 //    });
	...
	jQuery('#tabla-registros').on('click','a.btn-ver',function(e){
		...
		jQuery.ajax({
			...
			jQuery('#fecha_finalizacion').text(consulta.fecha_finalizacion);
			jQuery('#fecha_liquidacion').text(consulta.fecha_liquidacion);
