...
function iniciar()
{
	...
	function BD_Cargar(){
		...
		jQuery.ajax({
			url:'ws/travel/liquidacion/historial',	
			type:'POST', 	
			dataType: 'json',
			...
			jQuery.each(consulta, function (index, valor) { 
				if( valor.estado == 15 ){
					...
					row7 = '<i class="fa fa-check"> Completada</i>';

					row8 = '<a data-placement="top" title="Visualizar detalles" class="toltip btn-ver btn btn-info btn-xs" href="#modal-ver" data-toggle="modal" data-id="'+valor.id+'"><i class="fa fa-eye"></i></a> '+
							' <a data-placement="top" title="Ingresar el codigo" class="toltip btn-ingresar btn btn-success btn-xs" href="#modal-ingresar" data-toggle="modal" data-id="'+valor.id+'"><i class="glyphicon glyphicon-edit"></i></a>';
 ...
 //    });
	...
	jQuery('#tabla-registros').on('click','a.btn-ver',function(e){
		...
		jQuery.ajax({
			...
			jQuery('#pais_codigo').val(consulta.pais);
