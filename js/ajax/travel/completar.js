...
var ceco_validacion=false;
var orden_validacion=false;
...
{ 
	...
	function loadData()
	{
		...
		$.ajax({
			url: 		'ws/travel/liquidacion/historial/aprobadas',	
			type: 		'GET', 	
			dataType: 	'json',
			...
			if (response.result) {
				if (response.records.length > 0) {
					...
					$.each(response.records, function (index, value) { 
						...
						row7 = formato_fecha(value.fecha_aprobacion);
						row8 = value.nombre_aprobador;
						if (value.estado == 1) {
							row9 = '<span class="label label-success"> Aprobada </span>';
						} else {
							row9 = '<span class="label label-danger"> Rechazada </span>';
						}
						// row9 = '<span class="label label-success"> Aprobada </span>';
						row10 = '<a data-placement="top" title="Ver" class="toltip btn btn-info btn-xs btn-liq-detail" data-dismiss="modal" data-usuario="'+value.usuario_creo+'" data-id="'+value.id+'" data-index="'+index+'"><i class="fa fa-eye"></i> Ver</a>';
		                window.tabla_registros.row.add([row1,row2,row3,row4,row5,row6,row7,row8,row9,row9,row10,row7,]).draw(false);
					});
					//console.log(response.records);
				} else {
    ...
    {
    	// console.log((localStorage.TRAVEL_DATOS_USUARIO));
    	$('#loader').fadeOut();
    	$('#gastos-list').html('');
    ...
    // 				// else {
				...
				if (response.records.gastos.length == 0) {
					...
					$.each(response.records.gastos, function (index, value) {
						console.log(response.records.gastos);
						if (value.estado == 1) {
							var html = 	'<div class="panel panel-info detail-expense">'+
										'<div class="panel-heading" style="font-size: 1.6em;">'+
											'<input id="'+value.id+'" type="checkbox" data-id="'+value.id+'" class="check-expense" checked disabled/>'+
							...
							var html = 	'<div class="panel panel-info detail-expense">'+
										'<div class="panel-heading" style="font-size: 1.6em;">'+
											'<input id="'+value.id+'" type="checkbox" data-id="'+value.id+'" class="check-expense" disabled/>'+
									  		'<label for="'+value.id+'" class="checkbox">Gasto #'+value.correlativo+'      '+
									  		'<input type="button" id="btn-editar-gasto" class="validacion btn btn-primary" data-id="'+value.id+'" style="float:right;" value="Editar Gastos"> </label>'+
										'</div>'+
										'<div class="panel-body">'+
				...
				$('#gastos-list .validacion').attr('disabled','true');
				$("#modal-ver").modal();
    	...
    	if (true) {
	    	...
	    	// } else if (contador >= array.length) {
	    		...
	    		// } else{
		    		...
		    		$.ajax({
		    			type: 		'POST',
		    			url: 		'ws/travel/liquidacion/aprobarSAP',
		    			dataType: 	'json',
		    			data: 		data
		    		})
		    		.done(function (response) {
	...
	jQuery('#btn-rechazar-enviar').on('click', function(e){//--------------------------------------------------->enviar el rechazo 
		...
		if( jQuery('#coment').val() ){
			...
			jQuery.ajax({
				...
				type:'POST', 	
				dataType: 'json',
				data : {id_liquidacion: jQuery('#id_liquidacion').val(), tipo: jQuery('#tipo').val(), comentario_rechazado: jQuery('#coment').val(), json: jsons,pais: jQuery('#pais_codigo').val(),sociedad: jQuery('#sociedad_usuario_creo').val(),empid: jQuery('#empid').val(), hola:jQuery('#codigo').val('holass'), codigo: jQuery('#codigo').val(), acreedor:jQuery('#acreedor_us_cr').val()}
			}).done(function( data ){
				//console.log(data);
				if(data.result){
					BD_Cargar();
	        	...
	        	jQuery.each(consulta.gastos, function (index, valor) { 
				    ...
				    .done(function(data){
				    	...
				    	jQuery.each(data.records, function (index_GASTO, valor_gasto){
				    		if (valor_gasto.monton != valor_gasto.monto ) {
				    			modal_show = 1;
				    		} else {
				    			modal_show = 0;
				    		}
