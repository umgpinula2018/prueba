...
{ 
	...
	$('#btn-send-aprob').on('click', sendRejectLiquidation);
	// Functions
	...
	function changeCeco ()
    {
        ...
        if ($('#ceco'+id).val()) {
            ...
            $('#btn-buscar-orden').css('pointer-events', '');
        }
    ...
    function liquidationDetail () 
    {
    	...
    	.done(function (response) {
    		if (response.result) {
                ...
                $('#moneda').text(moneda);
				$('#fecha_finalizacion').text(formato_fecha(response.records.fecha_finalizacion));
				$('#fecha_liquidacion').text(formato_fecha(response.records.fecha_liquidacion));
										...
										'<div class="panel-body">'+
											...
											'<div class="col-lg-6">'+
												'<div class="input-group">'+
													'<span class="input-group-addon btn-info">'+documento+'</span>'+
											  		'<input type="text" class="form-control" aria-describedby="basic-addon1" value="'+value.documento+'" disabled />'+
												'</div>'+
											'</div>'+
											'<div class="col-lg-6">'+
												'<div class="input-group">'+
													'<span class="input-group-addon btn-info"> Código tributario</span>'+
													'<input type="text" class="form-control" aria-describedby="basic-addon1" value="'+value.codigo_tributario+'" disabled />'+
												'</div>'+
											'</div>'+
											'<div class="form-group col-md-12" ></div>'+
											'<div class="col-lg-6">'+
												'<div class="input-group">'+
													'<span class="input-group-addon btn-info"> Órden</span>'+
											  		'<input id="orden" data-id="'+value.id+'" class="orden form-control orden'+value.id+'" aria-describedby="basic-addon1" value="'+value.orden+'" disabled />'+
											...
											'<div class="col-lg-6">'+
												'<div class="input-group">'+
													...
													'<input class="form-control" aria-describedby="basic-addon1" value="'+value.factura+'" disabled />'+
												'</div>'+
											'</div>'+
											'<div class="col-lg-6">'+
												'<div class="input-group">'+
													'<span class="input-group-addon btn-info"> Fecha</span>'+
													'<input class="form-control" type="date" aria-describedby="basic-addon1" value="'+formato_fecha2(value.fecha)+'" disabled/>'+
												'</div>'+
											'</div>'+
											'<div class="form-group col-md-12"></div>'+
											'<div class="col-lg-6">'+
    ...
    function sendAprobLiquidation () 
   	{
    	...
    	data = {
    		id_liquidacion: liquidacion_id,
    		tipo: 1,
    		comentario_rechazo: $('#coments').val(),
    		json: JSON.stringify(array),
    		empid: item_liquidacion.empid,
    		pais: item_liquidacion.pais,
    		acreedor: item_liquidacion.acreedor,
	...
	jQuery('#btn-rechazar-enviar').on('click', function(e){//--------------------------------------------------->enviar el rechazo 
		...
		if( jQuery('#coment').val() ){
			...
			jQuery.ajax({
				...
				type:'POST', 	
				dataType: 'json',
				data : {id_liquidacion: jQuery('#id_liquidacion').val(), tipo: jQuery('#tipo').val(), comentario_rechazado: jQuery('#coment').val(), json: jsons,pais: jQuery('#pais_codigo').val(),sociedad: jQuery('#sociedad_usuario_creo').val(),empid: jQuery('#empid').val(),codigo: jQuery('#codigo').val(), acreedor:jQuery('#acreedor_us_cr').val()}
			}).done(function( data ){
				console.log(data);
				if(data.result){
					BD_Cargar();
